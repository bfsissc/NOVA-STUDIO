...
async function tpInit() {
  if (!U || !U.email) { console.warn('TP: user not ready'); return; }

  // Always do a fresh connections load so render is up-to-date
  await tpLoadAllConnects();

  if (!TP_INITIALIZED) {
    TP_INITIALIZED = true;

    // ── Real-time listener for TP directory ──
    var _tpSnapUnsub = _tpCol().orderBy('createdAt','desc').onSnapshot(function(snap) {
      TP_DATA = snap.docs.map(function(d){ var o=d.data(); o.id=d.id; return o; });
      tpRenderList();
      tpRenderStats();
      tpRenderHomeWidget();
      // Background: load my feedback (don't block render)
      tpLoadMyFeeds().then(function(){ tpRenderList(); }).catch(function(){});
    }, function(err) {
      console.warn('TP snapshot error (trying fallback):', err.code || err);
      // Fallback 1: try with orderBy
      _tpCol().orderBy('createdAt','desc').get().then(function(snap) {
        TP_DATA = snap.docs.map(function(d){ var o=d.data(); o.id=d.id; return o; });
        tpRenderList(); tpRenderStats(); tpRenderHomeWidget();
        tpLoadMyFeeds().then(function(){ tpRenderList(); }).catch(function(){});
      }).catch(function() {
        // Fallback 2: no orderBy (in case index missing)
        _tpCol().get().then(function(snap) {
          TP_DATA = snap.docs.map(function(d){ var o=d.data(); o.id=d.id; return o; });
          TP_DATA.sort(function(a,b){ return (b.createdAt||0)-(a.createdAt||0); });
          tpRenderList(); tpRenderStats(); tpRenderHomeWidget();
        }).catch(function(e3){ tpToast('Could not load TP data: '+e3.message,'err'); });
      });
    });
    ...
    _tpConnCol().onSnapshot(function(snap) {
      ...
      console.warn('TP conn snapshot error:', e);
      tpRenderList(); tpRenderStats();
    });

  } else {
    // Already initialized — re-render with latest data
    tpRenderList();
    tpRenderStats();
    tpRenderHomeWidget();
  }
...
function tpRenderStats() {
  ...
  // Only count connections for TPs that exist in current user's directory
  var knownIds = {};
  TP_DATA.forEach(function(t){ knownIds[t.id]=true; });
  var totalConns = Object.keys(TP_ALL_CONNS).reduce(function(s,id){
    return s + (knownIds[id] ? TP_ALL_CONNS[id].length : 0);
  }, 0);
  // My connections: only ones pointing to known TPs
  var myConns = Object.keys(TP_MY_CONNECTS).filter(function(id){ return knownIds[id]; }).length;
...
async function tpToggleConnect(tpId) {
  if(!U||!U.email){ tpToast('You must be signed in.','err'); return; }
  ...
  if(TP_MY_CONNECTS[tpId]){
    ...
    try{
      ...
      tpToast('Disconnected ✓','ok');
    }catch(e){
      console.warn(e);
      // Roll back on error
      TP_MY_CONNECTS[tpId]=true;
      tpLoadAllConnects().then(function(){ tpRenderList(); tpRenderStats(); });
      tpToast('Error disconnecting: '+e.message,'err');
    }
  } else {
...
async function tpSaveWorkRole() {
  ...
  if(isNew) {
    ...
    try {
      ...
      TP_ALL_CONNS[tpId] = TP_ALL_CONNS[tpId].filter(function(c){ return c.userEmail !== U.email; });
      tpRenderList(); tpRenderStats();
      tpToast('Error connecting: '+e.message,'err');
    }
  } else {
    ...
    try {
      ...
      console.warn(e);
      tpToast('Error updating role: '+e.message,'err');
...
async function tpSaveTP(){
  ...
  var saveBtn = document.querySelector('#tpModal .btn.bl');
  if(saveBtn){ saveBtn.disabled=true; saveBtn.textContent='Saving…'; }

  try{
    if(editId){
      var tp=TP_DATA.find(function(t){return t.id===editId;});
      if(!tp||tp.ownerEmail!==U.email){
        tpToast('Permission denied.','err');
        if(saveBtn){ saveBtn.disabled=false; saveBtn.textContent='Save Partner'; }
        return;
      }
      // Optimistic local update
      var idx=TP_DATA.findIndex(function(t){return t.id===editId;});
      if(idx>-1) TP_DATA[idx]=Object.assign({},TP_DATA[idx],p);
      tpRenderList();
      ...
      p.ownerName=((U.firstName||'')+' '+(U.lastName||'')).trim()
        || (_authUser&&_authUser.displayName?_authUser.displayName.trim():'')
        || U.email;
      p.ownerAvatar=U.avatar||(_authUser&&_authUser.photoURL)||null;
      p.ownerUsername=U.username||null;
      p.createdAt=Date.now(); p.avgRating=0; p.ratingCount=0; p.connectionCount=0;

      // Optimistic local insert (prepend so it appears first)
      var tempId='__temp_'+Date.now();
      var tempEntry=Object.assign({},p,{id:tempId});
      TP_DATA.unshift(tempEntry);
      tpRenderList();
      tpCloseModal();

      // Persist to Firestore
      var docRef=await _tpCol().add(p);
      // Replace temp entry with real id
      var ti=TP_DATA.findIndex(function(t){return t.id===tempId;});
      if(ti>-1){ TP_DATA[ti].id=docRef.id; }
      tpToast('Training Partner added ✓','ok');
      tpRenderList();
      tpRenderStats();
      return; // modal already closed above
    }
    tpCloseModal();
  }catch(e){
    console.error(e);
    ...
    if(editId){
      // Snapshot will restore correct state; just re-fetch
      _tpCol().orderBy('createdAt','desc').get().then(function(snap){
        TP_DATA=snap.docs.map(function(d){var o=d.data();o.id=d.id;return o;});
        tpRenderList();
      }).catch(function(){});
      ...
      TP_DATA=TP_DATA.filter(function(t){return !t.id||t.id.indexOf('__temp_')===-1;});
      tpRenderList();
    }
  } finally {
    if(saveBtn){ saveBtn.disabled=false; saveBtn.textContent='Save Partner'; }
...
async function tpDelete(tpId,name){
  ...
  TP_DATA=TP_DATA.filter(function(t){return t.id!==tpId;});
  tpRenderList(); tpRenderStats();
  try{
    await _tpCol().doc(tpId).delete();
    tpToast('Deleted.','ok');
  }catch(e){
    tpToast('Error: '+e.message,'err');
...
async function tpSubmitFeedback(){
  ...
  var submitBtn = document.querySelector('#tpFeedbackModal .btn.bl');
  if(submitBtn){ submitBtn.disabled=true; submitBtn.textContent='Submitting…'; }

  try{
    var fbkRef=_tpFbkCol(tpId);
    var docRef;
    if(existId){ await fbkRef.doc(existId).update(fp); docRef={id:existId}; }
    else { docRef = await fbkRef.add(fp); }
    ...
    var newAvg=parseFloat(avg.toFixed(2));
    await _tpCol().doc(tpId).update({avgRating:newAvg,ratingCount:ratings.length});

    // Optimistic local update
    TP_MY_FEEDS[tpId]=Object.assign({id:docRef.id},fp);
    var tpIdx=TP_DATA.findIndex(function(t){return t.id===tpId;});
    if(tpIdx>-1){ TP_DATA[tpIdx].avgRating=newAvg; TP_DATA[tpIdx].ratingCount=ratings.length; }

    tpCloseFeedbackModal();
    tpRenderList();
    tpRenderStats();
    tpToast('Feedback submitted ✓','ok');
  }catch(e){
    console.error(e);
    tpToast('Error: '+e.message,'err');
  } finally {
